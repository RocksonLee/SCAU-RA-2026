...
static StaticTask_t Camera_thread_memory;
                ...
                static uint8_t Camera_thread_stack[8192] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t Camera_thread_stack[8192] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.Camera_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
void Camera_thread_create(void) {
	...
	Camera_thread = xTaskCreateStatic(
                    ...
                    #endif
			Camera_thread_func, (const char*) "Camera_thread", 8192 / 4, // In words, not bytes
