...
uint8_t sub_0005_arena[kArenaSize_sub_0005]
    BSP_PLACE_IN_SECTION(".sdram_nocache") BSP_ALIGN_VARIABLE(32);
// Fast scratch arena not used for Ethos-U55
//  We will not create it for now and reuse the address of the other arena
// __attribute__((aligned(16))) static uint8_t sub_0005_fast_scratch[122880];
uint8_t* sub_0005_fast_scratch = sub_0005_arena;

static void sub_0005_clean_dcache(void) {
  SCB_CleanDCache_by_Addr((uint32_t *) sub_0005_arena,
                          (int32_t) sizeof(sub_0005_arena));
}

static void sub_0005_invalidate_dcache(void) {
  SCB_InvalidateDCache_by_Addr((void *) sub_0005_arena,
                               (int32_t) sizeof(sub_0005_arena));
}
...
int sub_0005_invoke(bool clean_outputs) {
  ...
  // Buffer sub_0005_model with size 3584 and address: 4294967295
  base_addrs[0] = (uint64_t)(uintptr_t)sub_0005_model_data;
  base_addrs_size[0] = sub_0005_model_data_size;
  // Buffer sub_0005_arena with size 122880 and address: 0
  base_addrs[1] = (uint64_t)(uintptr_t) (sub_0005_arena+0);
  base_addrs_size[1] = 122880;

  // Buffer sub_0005_fast_scratch with size 122880 and address: 0
  ...
  base_addrs[3] = (uint64_t)(uintptr_t) (sub_0005_arena+73728);
